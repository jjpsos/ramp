		...
		if ( class_exists( 'Sliced_Admin_Notices' ) ) {
			...
			if ( ! $this->license_key > '' ) {
				...
				if ( ! Sliced_Admin_Notices::has_notice( $this->slug . '_license_expired' ) ) {
					...
					$notice_args = array(
						'class' => 'notice-error',
						'content' => '<p>' . sprintf( __( 'Dismiss message1.', $this->text_domain ), 'Sliced Invoices ' . $this->name, 'https://slicedinvoices.com/checkout/?edd_license_key='.$this->license_key.'&download_id='.$this->download_id ) . '</p>',
						'dismissable' => true,
						'dismiss_transient' => '2592000',
					);
				...
				if ( ! Sliced_Admin_Notices::has_notice( $this->slug . '_license_disabled' ) ) {
					...
					$notice_args = array(
						'class' => 'notice-error',
						'content' => '<p>' . sprintf( __( 'Dismiss message2.', $this->text_domain ), 'Sliced Invoices ' . $this->name, 'https://slicedinvoices.com/contact-us/' ) . '</p>',
						'dismissable' => false,
					);
